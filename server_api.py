...
def api():
    ...
    if not row:
        ...
        VALUES(?,?,?,?,?,?)
        """, (device, server, model, ip, now, now))
        conn.commit()
        conn.close()
    ...
    c.execute(
        "UPDATE devices SET last_seen=?, server=?, model=?, ip=? WHERE device_id=?",
        (now, server, model, ip, device)
    )
    conn.commit()
    conn.close()
...
def admin():
    ...
    active_count = sum(1 for r in rows if r[7] == 1 and r[8] == 0 and not expired(r[9]))
    blocked_count = sum(1 for r in rows if r[8] == 1)
    pending_count = sum(1 for r in rows if r[7] == 0 and r[8] == 0)
    expired_count = sum(1 for r in rows if expired(r[9]))
    ...
    *{box-sizing:border-box;margin:0;padding:0;}
    ...
    <div class="topbar">
        ...
        <div class="search-box">
            <input id="deviceSearch" onkeyup="filterDevices()" type="text" placeholder="Search by device, server, model or IP...">
        </div>
    </div>
...
def deactivate():
    ...
    conn = db()
    c = conn.cursor()
    c.execute("UPDATE devices SET is_active=0 WHERE device_id=?", (device,))
    conn.commit()
    conn.close()
    return Response('<script>window.location.href="/admin";</script>', mimetype="text/html")
...
def unblock():
    ...
    conn = db()
    c = conn.cursor()
    c.execute("UPDATE devices SET is_blocked=0 WHERE device_id=?", (device,))
    conn.commit()
    conn.close()
    return Response('<script>window.location.href="/admin";</script>', mimetype="text/html")
